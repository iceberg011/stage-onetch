...
public interface PayloadRepository extends JpaRepository<payload, Integer> {
    ...
    // ===== FIND BY PUNCH STATE =====
    List<payload> findByPunchstateTrue();
    List<payload> findByPunchstateFalse();
    
    // ===== FIND BY WORK CODE =====
    List<payload> findByWorkcode(String workcode);
    
    // ===== FIND BY EMPLOYEE AND PUNCH STATE =====
    List<payload> findByEmpidAndPunchstateTrue(Integer empid);
    
    // ===== FIND BY TIME CARD =====
    List<payload> findByTimecardid(Integer timecardid);
